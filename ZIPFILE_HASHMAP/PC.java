    ...
    public void play(Scanner s, PC pc) {
        ...
        while (gameon && respect < 80 && respect > 0) {
            ...
            if (words.length < 2) {
                ...
                if (this.room.Creatures.get(words[0]) != null) {
                    if (words[1].equalsIgnoreCase("clean")) {
                        System.out.println(this.name + " has forced " + this.room.Creatures.get(words[0]) + " to clean the room.");
//                        room.changingState("clean", pc);
                        if (!words[1].equalsIgnoreCase(this.room.Creatures.get(words[0]).preference())) {
                            if (!room.getState().equalsIgnoreCase(words[1])) {
                                this.room.Creatures.get(words[0]).discontent(pc);
                                this.room.Creatures.get(words[0]).discontent(pc);
                                room.changingState("clean", pc);
                            } else {
                                
                                room.changingState("clean");
                            }

                        } else {
                            if (!room.getState().equalsIgnoreCase(words[1])) {
                                this.room.Creatures.get(words[0]).gladness(pc);
                                this.room.Creatures.get(words[0]).gladness(pc);
                                room.changingState("clean", pc);
                            } else {
                        ...
                        System.out.println(this.name + " has forced " + this.room.Creatures.get(words[0]) + " to dirty the room.");
//                        room.changingState("dirty", pc);
                        if (!words[1].equalsIgnoreCase(this.room.Creatures.get(words[0]).preference())) {
                            if (!room.getState().equalsIgnoreCase(words[1])) {
                                this.room.Creatures.get(words[0]).discontent(pc);
                                ...
                                room.changingState("dirty");
                            }

                        } else if (words[1].equalsIgnoreCase(this.room.Creatures.get(words[0]).preference())) {
                            if (!room.getState().equalsIgnoreCase(words[1])) {
                                this.room.Creatures.get(words[0]).gladness(pc);
                                this.room.Creatures.get(words[0]).gladness(pc);
                                room.changingState("dirty", pc);
                        ...
                        if (this.room.east != null) {
                            System.out.println(this.name + " has forced " + this.room.Creatures.get(words[0]) + " to move to room " + room.east.getName());
                            this.room.Creatures.get(words[0]).changeRoom(room.east);
                        } else {
                            System.out.println("East room does not exist.");
                        }
                        ...
                        if (this.room.north != null) {
                            System.out.println(this.name + " has forced " + this.room.Creatures.get(words[0]) + " to move to room " + room.north.getName());
                            this.room.Creatures.get(words[0]).changeRoom(room.north);
                        } else {
                            System.out.println("North room does not exist.");
                        }
                    } else if (words[1].equalsIgnoreCase("west")) {
                        if (this.room.west != null) {
                            System.out.println(this.name + " has forced " + this.room.Creatures.get(words[0]) + " to move to room " + room.west.getName());
                            this.room.Creatures.get(words[0]).changeRoom(room.west);
                        } else {
                            System.out.println("West room does not exist.");
                        }
                    } else if (words[1].equalsIgnoreCase("south")) {
                        if (this.room.south != null) {
                            System.out.println(this.name + " has forced " + this.room.Creatures.get(words[0]) + " to move to room " + room.south.getName());
                            this.room.Creatures.get(words[0]).changeRoom(room.south);
                        } else {
                            System.out.println("South room does not exist.");
                        }
                    } else {
